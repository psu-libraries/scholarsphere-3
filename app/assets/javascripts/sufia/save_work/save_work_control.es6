// Overrides Sufia to ensure the form is not valid until all files have been uploaded
...
export class SaveWorkControl {
  ...
  isValid() {
    ...
    // This ensures uploads are complete before the Save button is made available
    let isComplete = $('tr.template-upload').length == 0
    return metadataValid && filesValid && agreementValid && isComplete
  }
