$(function() {
  ...
  for (var i=0; i < autocomplete_vocab.url_var.length; i++) {
    ...
    // need to update when the item is showing if it is hidden for autocomplete to work
//    $("#collapse_" + autocomplete_vocab.url_var[i]).click( function() {
//       var label = this.id.replace("collapse_","");
//       $("#generic_file_"+label).autocomplete(get_autocomplete_opts(label));
//    });
  ...
  function setup_autocomplete(obj, cloneElem) {
    ...
    if (obj.id == 'additional_based_near_submit') {
      ...
      $($(obj).parent()[0]).find('input[type=text]').autocomplete(get_autocomplete_opts(autocomplete_vocab.url_var[index]));
    }
  }
  ...
  $('form').multiForm({afterAdd: setup_autocomplete});
    ...
    // events for autocomplete and create other array values for autocomplete
    for (var i=0; i < autocomplete_vocab.url_var.length; i++) {
        var label = autocomplete_vocab.url_var[i];
        $("#collapse_" + label).height('auto')
        $("#generic_file_"+label).autocomplete(get_autocomplete_opts(label));

    }

    /*    $("#collapse_subject").click( function() {
            var label = this.id.replace("collapse_","");
            $("#generic_file_"+label).autocomplete(get_autocomplete_opts(label));
        });
        $("#collapse_language").click( function() {
            $("#generic_file_language").autocomplete(get_autocomplete_opts("language"));
        });*/
