...
class PersistRemoteContainedOutputFileService < Hydra::Derivatives::PersistBasicContainedOutputFileService
  ...
  def self.call(stream, directives)
    container = directives.fetch(:container)
    ...
    if ENV['REPOSITORY_EXTERNAL_FILES'] == 'true'
      ...
      filepath = pairtree.create_repository_files_from_string(stream, 'extracted_text.txt')
      ...
      remote_file = file_set.send("build_#{container}".to_sym)
