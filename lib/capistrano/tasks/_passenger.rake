...
namespace :passenger do
  ...
  task :install do
   on roles(:web)  do
    ...
    execute "RBENV_VERSION=#{fetch(:rbenv_ruby)} gem install --source https://download:#{passenger_token}@www.phusionpassenger.com/enterprise_gems/ passenger-enterprise-server --no-ri --no-rdoc -v 5.0.6"
